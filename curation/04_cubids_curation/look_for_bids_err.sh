# look for cases where there is a standard scan and a run-01 scan
...
# look for cases where there is a run-01 but no run-01
for subdir in sub-*; do
  anat_dir="$subdir/ses-1/func"
  if [ -d "$anat_dir" ]; then
    has_run01=$(find "$anat_dir" -maxdepth 1 -name '*_ses-1_task-face_acq-singleband_bold.json' | wc -l)
