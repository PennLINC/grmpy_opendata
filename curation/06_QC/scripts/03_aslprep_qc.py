...
inpath_qc = os.path.join(
    project_path, "data/derivatives/aslprep/"
)  # CUBIC project path - replace
outpath = os.path.join(
    project_path, "code/curation/06_QC/data/"
)  # CUBIC project path - replace
...
fileNames_qc = sorted(
    glob.glob(
        os.path.join(
            inpath_qc, "sub-*/ses-*/perf/", "sub-*_ses-*_desc-qualitycontrol_cbf.tsv"
        )
    )
)
...
# ----------------------
qc_output_path = os.path.join(outpath, "aslprep_qc.csv")
